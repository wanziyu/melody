...
func getConditionInference(inference *melodyv1alpha1.Inference, condType melodyv1alpha1.ServingStatusType) *melodyv1alpha1.InferenceCondition {
	for _, condition := range inference.Status.Conditions {
...
func newConditionInference(conditionType melodyv1alpha1.ServingStatusType, status v1.ConditionStatus, message string) melodyv1alpha1.InferenceCondition {
	return melodyv1alpha1.InferenceCondition{
		Type:               conditionType,
		Status:             status,
...
func SetConditionInference(inference *melodyv1alpha1.Inference, conditionType melodyv1alpha1.ServingStatusType, status v1.ConditionStatus, name string) {
	...
	inference.Status.Conditions = append(inference.Status.Conditions, newCond)
}

func removeConditionInference(inference *melodyv1alpha1.Inference, condType melodyv1alpha1.ServingStatusType) {
	var newConditions []melodyv1alpha1.InferenceCondition
	for _, c := range inference.Status.Conditions {
		if c.Type == condType {
			continue
		}
		newConditions = append(newConditions, c)
	}
	inference.Status.Conditions = newConditions
}
...
func GetLastConditionType(inference *melodyv1alpha1.Inference) (melodyv1alpha1.ServingStatusType, error) {
	if len(inference.Status.Conditions) > 0 {
		return inference.Status.Conditions[len(inference.Status.Conditions)-1].Type, nil
	}
